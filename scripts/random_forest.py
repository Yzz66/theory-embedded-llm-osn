"""
Random Forest baseline for ontological security narrative classification.

This script trains a TF-IDF + RandomForestClassifier model on a fixed training
set and evaluates it on a fixed test set. Random Forest includes stochastic
components, so the default number of runs is 5.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run TF-IDF + Random Forest train-test evaluation."
    )
    parser.add_argument(
        "--train-path",
        required=True,
    ...
    parser.add_argument(
        ...
        help="Number of repeated train-test runs. Default is 5 for Random Forest.",
    )
    parser.add_argument(
        "--random-state",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of parallel jobs for Random Forest.",
    )
    parser.add_argument(
        "--min-df",
        type=int,
...
def build_pipeline(args: argparse.Namespace, seed: int) -> Pipeline:
...
def write_text_report(
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write("Random Forest Baseline: Train-Test Evaluation\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write("Classifier: RandomForestClassifier\n")
...
def main() -> None:
    ...
    txt_path = output_dir / f"random_forest_train_test_results_{timestamp}.txt"
    write_text_report(
        txt_path,
        args,
        train_df,
    ...
    pred_xlsx_path = output_dir / f"random_forest_train_test_predictions_{timestamp}.xlsx"
    pred_df.to_excel(pred_xlsx_path, index=False)

    metrics_xlsx_path = output_dir / f"random_forest_train_test_metrics_{timestamp}.xlsx"
