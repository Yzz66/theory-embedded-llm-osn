"""
KNN baseline for ontological security narrative classification.

This script trains a TF-IDF + KNeighborsClassifier model on a fixed training set
and evaluates it on a fixed test set. Under the default configuration, the
method is deterministic, so the default number of runs is 1.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run TF-IDF + KNN train-test evaluation."
    )
    parser.add_argument(
        "--train-path",
        required=True,
    ...
    parser.add_argument(
        ...
        help="Base random seed used only when shuffling the training set.",
    )
    parser.add_argument(
        "--shuffle-train-each-run",
        action="store_true",
        help=(
            "Shuffle the training set before each run. This does not change the "
            "train-test split and only affects rare KNN tie cases."
        ),
    )
    parser.add_argument(
        "--n-neighbors",
    ...
    parser.add_argument(
        "--uniform-weights",
        action="store_true",
        help="Use uniform KNN weights instead of distance weights.",
    )
    parser.add_argument(
        "--min-df",
        type=int,
        ...
        [
            ...
            (
                ...
                KNeighborsClassifier(
                    ...
                    weights="distance" if use_distance_weights else "uniform",
                    metric="cosine",
                ),
            ),
        ]
...
def resolve_label_order(y_train: List[str], y_test: List[str]) -> List[str]:
    detected = sorted(set(y_train) | set(y_test))
    if set(DEFAULT_LABEL_ORDER).issubset(set(detected)):
...
def write_text_report(
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write("KNN Baseline: Fixed Train-Test Evaluation\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write("Classifier: KNeighborsClassifier\n")
        f.write("ngram_range: (1, 2)\n")
        f.write(f"min_df: {args.min_df}\n")
        f.write(f"max_df: {args.max_df}\n")
        f.write(f"n_neighbors: {args.n_neighbors}\n")
        f.write(f"weights: {'uniform' if args.uniform_weights else 'distance'}\n")
        f.write("metric: cosine\n")
        f.write(f"N_RUNS: {args.n_runs}\n")
        f.write(f"Shuffle training set each run: {args.shuffle_train_each_run}\n\n")

        f.write("[3] Summary Metrics Across Runs: Mean ± Std\n")
        f.write("-" * 70 + "\n")
...
def main() -> None:
    ...
    y_train_base = train_df[args.label_col].astype(str).str.strip().tolist()
    x_test = test_df[args.text_col].astype(str).tolist()
    y_test = test_df[args.label_col].astype(str).str.strip().tolist()

    labels = resolve_label_order(y_train_base, y_test)

    print(f"Train size: {len(train_df)}")
    print(f"Test size: {len(test_df)}")
    print(f"Detected labels: {labels}")
    print(f"Train label distribution: {Counter(y_train_base)}")
    ...
    for run_id in range(1, args.n_runs + 1):
        ...
        if args.shuffle_train_each_run:
            shuffled_train_df = shuffle(train_df, random_state=seed).reset_index(drop=True)
            x_train = shuffled_train_df[args.text_col].astype(str).tolist()
            y_train = shuffled_train_df[args.label_col].astype(str).str.strip().tolist()
        else:
        ...
        pipeline = build_pipeline(
            ...
            use_distance_weights=not args.uniform_weights,
            min_df=args.min_df,
            max_df=args.max_df,
        )
    ...
    xlsx_path = output_dir / f"knn_train_test_predictions_{timestamp}.xlsx"
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        predictions_df.to_excel(writer, index=False, sheet_name="predictions_all_runs")
        metrics_df.to_excel(writer, index=False, sheet_name="run_metrics")
